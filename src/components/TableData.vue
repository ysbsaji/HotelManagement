<template>
  <div>
    ...
    <template v-slot:[`item.status`]="{ item }">
      <p style="color: #29B6F6" v-if="item.status === 'Booked'" class="mt-3"> <u v-for="(list, index) in data.bookedList" :key="index" @click="list.click(item)">{{ item.status }}</u> </p>
      <p v-else class="mt-3"> {{ item.status }}</p>
    </template>
    ...
    <template v-slot:[`item.isAssigned`]="{ item }">
      <v-row>
        <v-col v-if="item.status !== 'Free'" class="mt-3">
          <label v-for="(list, index) in data.isAssignedList" :key="index" text>
            <label v-if="item.isAssigned">{{ item.isAssigned }}<v-icon  @click="list.click(item)" v-if="$store.state.userDetails.role === 'Manager'">mdi-check</v-icon> </label>
